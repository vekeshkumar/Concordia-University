...
public class RM03 {
	...
	public static void main(String[] args) {
	    ...
	    new Thread(new StartServer("QUE", "9030")).start();
		new Thread(new StartServer("MTL", "9040")).start();
		new Thread(new StartServer("SHE", "9050")).start();
	}
}
...
class ExecuteRequest implements Runnable {
	...
	public ExecuteRequest(String RMID, RM03 obj, int startingSeqNo) {
		this.RMID = RMID;
		this.obj = obj;
	...
	public void UDPUniCastSend(Result ResultObj) {
		...
		int serverPort;
		try {
			aSocket = new DatagramSocket(8005);
