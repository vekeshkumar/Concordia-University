...
import EventApp02.EventServer02POA;

public class EventManagement extends EventServer02POA{
	private ORB orb;
	...
	public synchronized void logging(String infoMsg) {
		...
		// Write eventDB
		File file = new File("C:/Java/EclipseWorkspace/COMP6231_Project/src/ReplicaManager02/" + this.serverName + "_eventDB.txt");
		...
		//Write Customer DB
				file = new File("C:/Java/EclipseWorkspace/COMP6231_Project/src/ReplicaManager02/" + this.serverName +"_custDB.txt"); 
				...
				try(BufferedWriter br = new BufferedWriter(new FileWriter(file))){
					for(String custID: custDB.keySet()) {
						...
						for (String eventType : custDB.get(custID).keySet()) {
							for(String eventID: eventDB.get(eventType).keySet()) {
								br.write(eventType);
								br.write("-");
								br.write(eventID);
								br.write(",");
