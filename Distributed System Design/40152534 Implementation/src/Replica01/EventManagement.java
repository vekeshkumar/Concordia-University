...
import EventApp.EventServerPOA;

public class EventManagement extends EventServerPOA{
	public static boolean sendCorrectResult;
	private ORB orb;
	...
	public EventManagement(String serverName){
		...
		//For logging
		//filename  = "C:/Java/EclipseWorkspace/COMP6231_Project/src/Replica01/" +this.serverName +"_Server.txt";
		filename  = "G:/workspace/COMP6231_Project/src/Replica01/" +this.serverName +"_Server.txt";
		...
		//getLogfilename = "C:/Java/EclipseWorkspace/COMP6231_Project/src/ReplicaManager01/" + this.serverName;
		getLogfilename = "G:/workspace/COMP6231_Project/src/ReplicaManager01/" + this.serverName;
		
	}
	//Display Databases...
	public void displayDB() {
		System.out.println("Initial "+this.serverName +" Event DB:" +eventDB + " - Cust DB:" +custDB);
	}
	...
	public String addEvent(String eventID, String eventType, int bookingCapacity) {
		...
		if (!sendCorrectResult) bookingCapacity += 2;
		...
		if (bookingCapacity < 0) {
			addResult = "Cannot add negative capacity";
			logging("Add event - " + eventType +" - "+ eventID +" - " +bookingCapacity +" - " +addResult);
			return addResult;
		}
		synchronized(this) {
		...
		//Simulate Software Failure..
		if (sendCorrectResult) return "Success";
		else return "Failure";
		//return "Success";
	}
	...
	public String swapEvent(String customerID, String newEventID, String newEventType, String oldEventID, String oldEventType) {
		...
		if (custDB.get(customerID) == null || custDB.get(customerID).get(oldEventType) == null || custDB.get(customerID).get(oldEventType).get(oldEventID) == null) {
			...
			logging("Swap Event- " + customerID +" - " +newEventType +" - " +newEventID +" - " +oldEventType +" - " +oldEventID +" - " +swapResult);
			return swapResult;
		}
		...
		if (custDB.get(customerID) != null && custDB.get(customerID).get(newEventType) != null && custDB.get(customerID).get(newEventType).get(newEventID) != null) {
			...
			if (noofremoteBookings >= 3 && (!remoteEventList.contains(oldEventID))) bookResult = "You cannot book more than 3 events from other cities overall in a week";
				...
				if (noofremoteBookings >= 3) System.out.println("Old event falls in New event week. So can be swapped..");
		...
		if (bookResult.trim().equals("Success")) { //Cancel will be successful with the first check done above.
			...
			//If old event is not cancelled, Cancel the new event.
			if (!(swapResult.trim().equals("Success"))) {
				cancelEvent(customerID, newEventID, newEventType);
				swapResult = swapResult + ". Swap is unsuccessful.";
			}
		}
		else { //Do not cancel the old event.
			swapResult = bookResult + ". Swap is unsuccessful.";
		}
		
		System.out.println("Event DB after swap: " +eventDB + " - Cust DB after swap: " +custDB);
		logging("Swap Event- " + customerID +" - " +newEventType +" - " +newEventID +" - " +oldEventType +" - " +oldEventID +" - " +swapResult);
		return swapResult;
	}
	
	//UDP Client of 'this' server
	public String UDPClient(String[] args) {
		/*
		 * [0] - Server to call
		 * [1] - Server to call
		 * [2] - Method name
		...
		try {
			...
			for(i=0;i<2; i++) {
				...
				if (args[i].equals("QUE") ) {serverPort = 9000; requestdata[0] = args[i];}
				else if (args[i].equals("MTL")) {serverPort = 9001; requestdata[0] = args[i];}
				else if (args[i].equals("SHE")) {serverPort = 9002; requestdata[0] = args[i];}
	...
	/*	logger.addAppender(appender);
		...
		// Write eventDB
		File file = new File(getLogfilename + "_eventDB.txt");
		...
		} catch (IOException e) {System.out.println("unable to write the file.." + file.toString());}

		// Write Customer DB
		file = new File(getLogfilename + "_custDB.txt");

		try (BufferedWriter br = new BufferedWriter(new FileWriter(file))) {
			for (String custID : custDB.keySet()) {
				br.write(custID);
				br.write(" ");

				for (String eventType : custDB.get(custID).keySet()) {
					for (String eventID : custDB.get(custID).get(eventType).keySet()) {
						br.write(eventType);
						br.write("-");
						br.write(eventID);
						br.write(",");
	...
	//bookRemote Event
	public String bookRemoteEvent(String customerID, String eventID, String eventType) {
	...
	public void UDPServer(String args[]){
		...
		try {
			...
			sSocket = new DatagramSocket(Integer.parseInt(args[1]));
			...
			while(true) {
				...
				System.out.println("\nRequest#"+count +" Received = " +new String(request.getData()).trim() +" - from Port no." +request.getPort());
				count++;
				ServerResponder SP = new ServerResponder(sSocket,request,this);
				executor.execute(SP);
			}
		}
