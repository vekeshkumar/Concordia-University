...
public class RM01 {
	...
	public static void main(String[] args) {
		...
		String RMpath =  "G:/workspace/COMP6231_Project/src/ReplicaManager01/"; 
		...
		new Thread(new StartServer("QUE01", "9000", false)).start();
		new Thread(new StartServer("MTL01", "9001", false)).start();
		new Thread(new StartServer("SHE01", "9002", false)).start();
	}
}
	...
	public void run() {
		...
		while (true) {
			...
			try {
				...
				//Start Servers...
				new Thread(new StartServer("QUE01", "9000", true)).start();
				new Thread(new StartServer("MTL01", "9001", true)).start();
				new Thread(new StartServer("SHE01", "9002", true)).start();
	...
	public void run() {
		...
		while (true) {
			...
			case 9998:
				...
				if (requestObj.isConsecutiveswFailure() && failedRM.equals(RMID)) EventManagement.sendCorrectResult = true;
				obj.executedMap.put(seqNo, requestObj);
				break;
