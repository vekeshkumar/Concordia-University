...
public class FrontEndManagement extends FrontEndServerPOA{
	...
	//Maintains latest result for each SeqNo..
	Map<Integer, String> ResultMap = new TreeMap<Integer, String>();
	Map<String,Integer> swFailureRecord = new HashMap<String,Integer>();  //RMID, Count..
	...
	public FrontEndManagement() {
		try {
			...
			// Pre-Populate swFailureRecord
			swFailureRecord.put("RM01", 0);
			swFailureRecord.put("RM02", 0);
			swFailureRecord.put("RM03", 0);
	...
	public int UDPUniCastReceive() {
		...
		try {
			...
			while(true) {
				...
				if (isFirstReq == true) aSocket.setSoTimeout((int) dynamicTime);
				else {
				...
				if (isFirstReq == true) {  //No Process Crash..
					if (replyList.size() == 3) {
						...
						if (failedRMID == null) {  //No Software Failure..
							...
							FrontEndDB.put(resultObj.getSeqNo(), frontendDataObj);			
							//ResultMap.put(resultObj.getSeqNo(), "SOFTWARE_FAILURE");
							isFirstReq = false;
							return resultObj.getSeqNo();
						}
					}
				}
				else {
