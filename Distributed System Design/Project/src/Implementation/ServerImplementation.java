	...
	public synchronized String listEventAvailability(String eventType) {
        ...
        for (String loc : locList) {
            if (loc== this.location) {            	
            	...
            	req[cntr] = new UDPIntReqClient(RM2.serverRepo.get(loc),eventType,"listing");
                req[cntr].start();
                cntr++;
            }
        }
	...
	public synchronized String bookEvent(String customerId, String eventId, String eventType) {
		...
		if(this.location.equalsIgnoreCase(eventServLocation)){
			...
			if(isBookingAllowed) {
				...
				UDPIntReqClient req = new UDPIntReqClient(RM2.serverRepo.get(eventServLocation),eventType,eventId,customerId,functionType);
				req.start();
				try {
					req.join();
				} catch (InterruptedException e) {
